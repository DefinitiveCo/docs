...
This page shows the full same-chain flow end to end (quote, sign, submit) as a runnable script for each chain. The EVM example buys WETH with USDC on Base; the Solana example buys WSOL with USDC.
...
Market orders do not accept a limit price (`limitNotionalPrice` or `limitCrossPrice`), `expireTime`, or `triggers`. The onchain deadline is set automatically (roughly 5 minutes from quote time).
...
<CardGroup cols={2}>
  <Card title="Crosschain Orders" icon="arrows-left-right" href="/crosschain-orders">
    Move funds across chains in one order.
  </Card>
