...
Trigger orders fire on a price condition rather than a fixed limit. Each entry in `triggers` carries a `notionalPrice` (the USD price of the target asset) and a `triggerType`: `lower` fires when the market drops to or below the price, `upper` fires when it rises to or above. For the concepts behind the parameters, see [Placing Orders](/placing-orders).
...
Each of these takes exactly one trigger. `qty` follows the usual rule: contra units on a buy, target units on a sell.
...
Add `limitNotionalPrice` to any of these for a limit variant (the order becomes a limit order once triggered), and `expireTime` (an ISO-8601 timestamp) for good-til-time behavior. Omit `expireTime` for good-til-cancelled.
