...
<AccordionGroup>
  ...
  <Accordion title="SVM (Solana)" defaultOpen>
    ...
    - `svm.delegateIx` *(optional)* — raw delegation instruction to submit onchain yourself; the default path for essentially every account
    - `svm.sponsoredDelegateTx` *(optional)* — base64 `VersionedTransaction`, a sponsor-paid alternative to `svm.delegateIx`; present only for the small set of accounts Definitive has enabled it for
  </Accordion>
</AccordionGroup>
