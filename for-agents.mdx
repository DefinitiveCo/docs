...
<Note>
You can spend a native gas asset two ways, and both settle against the wrapped token.

**Wrap first.** Wrap it yourself, then quote and submit against the wrapped token address like any other token.

**Quote native.** Quote with the native asset and Flash returns a `wrap` action to send from `funderAddress` before submitting.
...
See [EVM native assets](/evm-overview#native-eth-and-wrapped-assets) and [Solana native SOL](/solana-overview#native-sol-and-wsol).
</Note>
...
- `wrap` *(conditional)* — pre-trade wrapping action, present only when the quote spends a native gas asset
...
<Note>
To track the order after submit, either poll `GET /orders`, or stream live updates over the [Orders WebSocket](/api-reference/flash/websocket) instead of polling.
</Note>

## Supported chains

`arbitrum`, `avalanche`, `base`, `bsc`, `ethereum`, `optimism`, `polygon`, `solana`, `hyperevm`, `plasma`, `monad`, `robinhood`
