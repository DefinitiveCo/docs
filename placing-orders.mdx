...
**Cross** is the **pair rate**: how many `contraAsset` units one `targetAsset` unit is worth. It is the price of the target asset denominated in the contra asset — the same target/contra direction as notional, just quoted in the contra asset instead of dollars, and never inverted. With WETH as the target and USDC as the contra, a cross price of `"4000"` means 4,000 USDC per WETH; with cbBTC as the contra it would be cbBTC per WETH. The frame is the same on `buy` and `sell`. Set it with `limitCrossPrice` on a limit, or `crossPrice` on a trigger.
...
`limit` orders require a limit price in exactly one basis: `limitNotionalPrice` (USD price of the target asset) or `limitCrossPrice` (`contraAsset` units per one `targetAsset` unit). Sending both is rejected. Either one prices the traded asset, `targetAsset` — the contra asset is the unit of account, never the thing being priced.
...
A limit price is also optional on `twap`, `stop`, `stop-loss`, and `take-profit`, in either basis. On a TWAP each tranche rests at that price for its own time slice, carrying unfilled size into the next slice rather than pausing the schedule (see [how a limit price interacts with the schedule](/twap-order#how-a-limit-price-interacts-with-the-schedule)); on a trigger order it promotes the order to the corresponding limit variant. Only `market` rejects a limit price outright.

## Trigger orders

`stop`, `stop-loss`, and `take-profit` orders fire on a price condition rather than a fixed limit. Each entry in `triggers` carries exactly one of `notionalPrice` (the USD price of the target asset) or `crossPrice` (the pair rate: `contraAsset` units per one `targetAsset` unit) — sending both, or neither, is rejected — plus a `triggerType`: `lower` fires when the market drops to or below the price, `upper` fires when it rises to or above. Each takes exactly one trigger.
...
The same order as the limit buy above, priced in contra units per target unit instead of USD. Use this basis when the contra asset is not a dollar stablecoin, or when you want the constraint to track the pair rather than the dollar.
...
| `limitCrossPrice` | Required on limit orders (one basis); optional on TWAP and trigger orders | Pair rate: `contraAsset` units per one `targetAsset` unit, same frame on buy and sell; mutually exclusive with `limitNotionalPrice` |
| `triggers` | Stop / stop-loss / take-profit | Each entry takes exactly one of `notionalPrice` (USD price of the target asset) or `crossPrice` (`contraAsset` units per one `targetAsset` unit), plus `triggerType` (`upper` / `lower`) |
