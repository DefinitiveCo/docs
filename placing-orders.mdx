...
A limit price is also optional on `twap`, `stop`, `stop-loss`, and `take-profit`, in either basis. On a TWAP it bounds every tranche to that price or better; on a trigger order it promotes the order to the corresponding limit variant. Only `market` rejects a limit price outright.
