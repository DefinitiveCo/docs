...
| `limit`, `stop`, `stop-loss`, `take-profit` (including their limit variants when `limitNotionalPrice` is supplied) | Set `expireTime` for good-til-time (GTT) behavior. Omit it for good-til-cancelled (GTC), which uses an effectively non-expiring sentinel (`2^48 - 1`). |
...
| `triggers` | Stop / stop-loss / take-profit | `notionalPrice` is the USD price of the target asset (`targetAsset`), not the contra asset; plus `triggerType` (`upper` / `lower`) |
