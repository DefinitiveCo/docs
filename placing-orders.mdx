...
Crosschain execution supports `market` orders only. QuickTrade is not supported. See [Crosschain Orders](/crosschain-orders) for the request flow and example.
...
Once a limit order is resting, its price can be changed without cancelling and re-placing — see [Updating Orders](/updating-orders). The same applies to the limit price on a resting `stop-limit` or `take-profit-limit` order.
