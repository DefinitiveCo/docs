...
QuickTrade is a market-only execution mode for low-latency trading. Set `quickTrade: true` on both `POST /quote` and `POST /order` when instant quoting and faster execution matter more than additional route optimization.
