...
Definitive charges a base fee of 10 bps (0.10%) on every trade, across all order types. This is separate from your integrator fee: `flashIntegratorFeeBps` stacks on top, so the total a user pays is Definitive's 10 bps plus whatever rate you set. Both are already included in the quote's `estimatedFeeNotional`.
