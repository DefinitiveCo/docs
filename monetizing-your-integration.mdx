...
The codes are comma-separated, with Definitive's own registered code first and yours second — so on Base mainnet the codes payload ends with `,myapp`. Definitive only registers a code on Base, so on other EVM chains the suffix carries your code alone (`myapp`).
