...
| `durationSeconds` | Yes | Total duration in seconds. Minimum 300 (5 minutes) |
| `twapBucketCount` | Optional | Number of tranches, between 2 and 2560 |

TWAP orders do not accept `limitNotionalPrice`, `expireTime`, or `triggers`. Progress is surfaced on the order status through `twapBucketCount` and `completedFillCount`.
...
To buy instead, set `side` to `"buy"` and denominate `qty` in the contra asset (for example `"1000"` USDC). Add `twapBucketCount` to control how many tranches the order is split into.
